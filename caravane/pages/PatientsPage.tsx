...
const PatientsPage: React.FC = () => {
  ...
  const [specialistConsultedPatientIdsToExclude, setSpecialistConsultedPatientIdsToExclude] = useState<string[] | undefined>(undefined);
  ...
  const specialistCanStartOrEditConsultationPermission = isSpecialist && hasPermission('add_consultations');
  ...
  const fetchInitialDataForFilters = useCallback(async () => {
    if (isAccueil) {
      ...
      try {
        ...
        const relevantOrientations = allConsultationsResponse.data.filter(c => {
        ...
        const orientedPatientIds = Array.from(new Set(relevantOrientations.map(c => c.patient_id)));
        setSpecialistRelevantPatientIds(orientedPatientIds);

        const specialistConsultations = await consultationService.getConsultations({ medecinId: currentUser.id, limit: 100000 });
        const consultedIds = Array.from(new Set(specialistConsultations.data.map(c => c.patient_id)));
        setSpecialistConsultedPatientIdsToExclude(consultedIds);

      } catch (error) {
        console.error("Failed to fetch specialist relevant/consulted patient IDs:", error);
        setSpecialistRelevantPatientIds([]);
        setSpecialistConsultedPatientIdsToExclude([]);
      } finally {
  ...
  const fetchPatients = useCallback(async () => {
    ...
    if (isSpecialist && (
        (specialistRelevantPatientIds === undefined && !isLoadingSpecialistData) ||
        (specialistConsultedPatientIdsToExclude === undefined && !isLoadingSpecialistData) 
    )) return;
    ...
    try {
      ...
      if (isGP) {
        ...
        params.excludePatientIds = specialistConsultedPatientIdsToExclude; 
      }
      ...
      specialistConsultedPatientIdsToExclude 
    ]);

  useEffect(() => {
    if ((isAccueil && patientIdsToExcludeForAccueil !== undefined) ||
        (isGP && gpConsultedPatientIdsToExclude !== undefined) || 
        (isSpecialist && specialistRelevantPatientIds !== undefined && specialistConsultedPatientIdsToExclude !== undefined && !isLoadingSpecialistData) ||
        (!isAccueil && !isGP && !isSpecialist)) {
      fetchPatients();
    }
  }, [
      ...
      isSpecialist, specialistRelevantPatientIds, specialistConsultedPatientIdsToExclude, isLoadingSpecialistData,
      fetchPatients
    ]);
  ...
  const handleFormSubmit = async (patientData: PatientFormData, id?: string) => {
    ...
    try {
      ...
      await fetchInitialDataForFilters(); 
    } catch (error) {
    ...
    try {
      ...
      if (isGP) {
        ...
        params.forAccueilPatientList = true;
         ...
         if (specialistRelevantPatientIds && specialistRelevantPatientIds.length === 0) {
            setImportMessage({ type: 'info', text: 'Aucun patient à exporter pour ce spécialiste.' });
            setIsLoading(false);
            return;
        }
        params.patientIds = specialistRelevantPatientIds;
      ...
      if (result.data.length === 0) {
        setImportMessage({ type: 'info', text: 'Aucun patient à exporter avec les filtres actuels.' });
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        for (const row of jsonPatients) {
          ...
          if (Object.keys(validationErrors).length > 0) {
            ...
            errorDetails.push(`CIN ${patientCIN}: Validation échouée - ${errorsString}`);
            continue;
          }

          try {
            const existingPatients = await patientService.getPatients({ search: patientCIN, limit: 1 });
            let patientProcessedSuccessfully = false;

            if (existingPatients.data.length > 0 && existingPatients.data[0].cin === patientCIN) {
              // Patient exists, update
              const updatedPatient = await patientService.updatePatient(existingPatients.data[0].id, patientData);
              if (updatedPatient) patientProcessedSuccessfully = true;
            } else {
              // Patient does not exist, add
              const newPatient = await patientService.addPatient(patientData);
              if (newPatient) patientProcessedSuccessfully = true;
            }

            if (patientProcessedSuccessfully) {
              successCount++;
            } else {
              errorCount++;
              errorDetails.push(`CIN ${patientCIN}: Erreur lors de la création/mise à jour du patient.`);
            }
          } catch (processError: any) {
            errorCount++;
            errorDetails.push(`CIN ${patientCIN}: ${processError.message || 'Erreur inconnue lors du traitement.'}`);
          }
        }
        ...
        if (successCount > 0) await fetchInitialDataForFilters(); 
  ...
  if (isGP) {
    ...
    pageDescription = "Liste des patients qui vous ont été orientés mais pour lesquels vous n'avez pas encore démarré de consultation.";
  }


  const overallPageLoading = isLoading || isSubmitting ||
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-xl shadow-lg">
        ...
        {overallPageLoading && patients.length === 0 ? (
          <div className="flex justify-center items-center h-64">
            <LoadingSpinner
                message={
                    ...
                    (isSpecialist && isLoadingSpecialistData) ? "Recherche des patients et consultations Spécialiste..." :
                    "Chargement des patients..."
                }
                size="lg"
            />
          </div>
        ) : !overallPageLoading && patients.length === 0 && searchTerm ? (
          ...
          <div className="text-center py-10">
            ...
            <p className="text-gray-600 text-lg">
              {isGP ? "Aucun patient référé par l'accueil pour le moment." :
               ...
               isSpecialist ? `Aucun patient nécessitant une première consultation ne vous a été orienté.` :
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {patients.map((patient) => (
                  <PatientListItem
                    ...
                    isSpecialistContext={isSpecialist}
                    specialistCanStartOrEditConsultationPermission={specialistCanStartOrEditConsultationPermission}
                  />
                ))}
              </tbody>
            </table>
         ...
         {totalPages > 1 && !overallPageLoading && patients.length > 0 && (
          <Pagination
        ...
        >
            <div className="space-y-4">
                <p className="text-sm text-gray-600">
                    Sélectionnez un fichier Excel (.xlsx, .xls). Les patients existants (basé sur le CIN) seront mis à jour.
                    Les colonnes attendues sont: <code className="text-xs bg-gray-100 p-1 rounded">N° Unique</code> (sera ignoré),
                    <code className="text-xs bg-gray-100 p-1 rounded">Prénom</code>, <code className="text-xs bg-gray-100 p-1 rounded">Nom</code>,
                    <code className="text-xs bg-gray-100 p-1 rounded">CIN</code> (Clé), <code className="text-xs bg-gray-100 p-1 rounded">Adresse</code>,
