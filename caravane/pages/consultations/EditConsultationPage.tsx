...
import {
  Consultation, ConsultationFormData, ValidationErrors, UserRole, ConsultationType, ConsultationStatut, Specialite,
  Patient, Caravane, User, CaravaneStatut
} from '../../types';
...
import { SPECIALTIES_LIST, USER_ROLES_CONFIG, MOCK_USERS } from '../../constants';
...
const EditConsultationPage: React.FC = () => {
  ...
  const [displayPatientInfo, setDisplayPatientInfo] = useState<{ nom: string, prenom: string, cin: string } | null>(null);
  ...
  const canEditConsultation = isAdmin || hasPermission('edit_consultations_accueil') || (isGP && hasPermission('add_consultations')) || (isSpecialistUser && hasPermission('add_consultations'));
  ...
  const fetchDataForForm = useCallback(async () => {
    if (!consultationId) {
        setSubmitMessage({type: 'error', text: 'ID de consultation manquant.'});
        setIsDataLoading(false);
        return;
    }
    setIsDataLoading(true);
    try {
      const [consultationToEdit, patientsRes, caravanesRes, usersRes] = await Promise.all([
      ...
      if (!consultationToEdit) {
        setSubmitMessage({type: 'error', text: 'Consultation non trouvée.'});
        setFormData(null);
        setIsDataLoading(false);
        return;
      }

      if ((isGP || isSpecialistUser) && consultationToEdit.patient_id) {
        const patientDetails = await patientService.getPatientById(consultationToEdit.patient_id);
        if (patientDetails) {
          setDisplayPatientInfo({
            nom: patientDetails.nom,
      ...
      if(isGP || isSpecialistUser) fetchSpecialistWorkload();

      const formReadyData: ConsultationFormData = {
        patient_id: consultationToEdit.patient_id,
        caravane_id: consultationToEdit.caravane_id,
        medecin_id: consultationToEdit.medecin_id,
        type_consultation: consultationToEdit.type_consultation,
        specialite: consultationToEdit.specialite,
        notes: consultationToEdit.notes || '',
        diagnostic: consultationToEdit.diagnostic || '',
        orientation: consultationToEdit.orientation || '',
        oriented_to_medecin_id: consultationToEdit.oriented_to_medecin_id || undefined,
        oriented_to_specialties: consultationToEdit.oriented_to_specialties || [],
        hors_champ_specialite_caravane: consultationToEdit.hors_champ_specialite_caravane || false,
        scanner_effectue: consultationToEdit.scanner_effectue,
        orientation_chu: consultationToEdit.orientation_chu,
        suivi_necessaire: consultationToEdit.suivi_necessaire,
        statut: consultationToEdit.statut,
        date_rdv: consultationToEdit.date_rdv ? consultationToEdit.date_rdv.split('T')[0] : '',
        date_prochain_rdv: consultationToEdit.date_prochain_rdv ? consultationToEdit.date_prochain_rdv.split('T')[0] : undefined,
      };
      setFormData(formReadyData);

      setPatients(patientsRes.data);
      const currentCaravaneInList = caravanesRes.data.find(c => c.id === consultationToEdit.caravane_id);
      let finalCaravanes = caravanesRes.data.filter(c => c.statut === CaravaneStatut.EN_COURS || c.statut === CaravaneStatut.PLANIFIEE);
      if (currentCaravaneInList && !finalCaravanes.find(c=> c.id === currentCaravaneInList.id)) {
        finalCaravanes.push(currentCaravaneInList);
      }
      setCaravanes(finalCaravanes);
      ...
      console.error("Failed to fetch data for edit consultation form:", error);
      setSubmitMessage({type: 'error', text: 'Erreur de chargement des données du formulaire.'});
      setFormData(null);
    } finally {
      setIsDataLoading(false);
    }
  }, [consultationId, isGP, isSpecialistUser, fetchSpecialistWorkload]);

  useEffect(() => {
    if (canEditConsultation) {
      fetchDataForForm();
    }
  }, [canEditConsultation, fetchDataForForm]);


  if (!canEditConsultation && !isDataLoading) {
    return (
        <div className="text-center py-20 bg-white rounded-lg shadow-xl p-8">
            ...
            <p className="text-gray-500">Vous n'avez pas les permissions nécessaires pour modifier cette consultation.</p>
 ...
 const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData || !consultationId) return;

    setSubmitMessage(null);
    const finalFormDataSubmit = { ...formData };
    if (isGP) { 
        finalFormDataSubmit.type_consultation = ConsultationType.GENERALE;
        finalFormDataSubmit.specialite = Specialite.GENERALE;
        finalFormDataSubmit.statut = ConsultationStatut.TERMINEE; 
    } else if (isSpecialistUser) {
        finalFormDataSubmit.type_consultation = ConsultationType.SPECIALISEE;
        finalFormDataSubmit.specialite = currentUser?.specialite;
        finalFormDataSubmit.notes = ""; // Notes field is hidden for specialist
        if (!finalFormDataSubmit.suivi_necessaire) { // If suivi not necessary, clear date
            finalFormDataSubmit.date_prochain_rdv = undefined;
        }
    } else if (finalFormDataSubmit.type_consultation === ConsultationType.GENERALE) {
        finalFormDataSubmit.specialite = Specialite.GENERALE;
    }
    
    if (finalFormDataSubmit.date_rdv === '') finalFormDataSubmit.date_rdv = undefined;
    if (finalFormDataSubmit.date_prochain_rdv === '') finalFormDataSubmit.date_prochain_rdv = undefined;


    const validationErrors = validateConsultationForm(finalFormDataSubmit, false, isSpecialistUser); 
    setErrors(validationErrors);

    if (Object.keys(validationErrors).length === 0) {
      setIsLoading(true);
      try {
        await consultationService.updateConsultation(consultationId, finalFormDataSubmit);
        setSubmitMessage({type: 'success', text: 'Consultation mise à jour avec succès!'});
        ...
        setSubmitMessage({type: 'error', text: 'Erreur lors de la mise à jour de la consultation.'});
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const FormField: React.FC<{label: React.ReactNode, name: keyof ConsultationFormData, error?: string, children: React.ReactNode, isRequiredFromUser?: boolean, isHidden?: boolean}> = ({label, name, error, children, isRequiredFromUser, isHidden}) => {
    if (isHidden) return null;
    // For dynamic required, the asterisk will be part of the label prop if needed
    const isActuallyRequired = isRequiredFromUser ?? !!(validateConsultationForm(initialValidationCheckFormData, false, isSpecialistUser)[name]);
  ...
  if (isDataLoading) {
    return <div className="flex justify-center items-center h-64"><LoadingSpinner message="Chargement des données de la consultation..." size="lg"/></div>;
  }

  if (!formData && !isDataLoading) {
     return (
        <div className="text-center py-20 bg-white rounded-lg shadow-xl p-8">
            <i className="fas fa-exclamation-circle fa-3x text-orange-500 mb-4"></i>
            <h2 className="text-2xl font-semibold text-gray-700 mb-2">Consultation introuvable</h2>
            <p className="text-gray-500">{submitMessage?.text || `La consultation avec l'ID ${consultationId} n'a pas pu être chargée.`}</p>
            <Link to="/consultations" className="mt-6 inline-block px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition duration-150">
                Retour aux consultations
            </Link>
        </div>
  ...
  const pageTitle = isGP ? "Modifier Orientation" : (isSpecialistUser ? "Modifier Consultation Spécialiste" : "Modifier Consultation");


  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-semibold text-gray-800">{pageTitle} <span className="text-indigo-500 text-xl">#{consultationId?.substring(0,8)}</span></h2>
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg">
        <form onSubmit={handleSubmit} className="space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                <FormField label="Patient" name="patient_id" error={errors.patient_id} isRequiredFromUser={!isGP && !isSpecialistUser} isHidden={isGP || isSpecialistUser}>
                ...
                <FormField label="Caravane" name="caravane_id" error={errors.caravane_id} isRequiredFromUser={!isGP && !isSpecialistUser} isHidden={isGP || isSpecialistUser}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                <FormField label="Médecin Traitant" name="medecin_id" error={errors.medecin_id} isRequiredFromUser={!isGP && !isSpecialistUser} isHidden={isGP || isSpecialistUser}>
                 ...
                 <FormField label="Type de Consultation" name="type_consultation" error={errors.type_consultation} isRequiredFromUser={!isGP && !isSpecialistUser} isHidden={isGP || isSpecialistUser}>
            ...
            <FormField label="Spécialité Demandée" name="specialite" error={errors.specialite} isRequiredFromUser={!isGP && !isSpecialistUser && formData.type_consultation === ConsultationType.SPECIALISEE} isHidden={isGP || isSpecialistUser || formData.type_consultation !== ConsultationType.SPECIALISEE}>
                    ...
                    <div className="mt-2 space-y-2 sm:space-y-0 sm:flex sm:flex-wrap sm:gap-x-6 sm:gap-y-2">
                        {orientationSpecialtiesForCheckboxes.map(spec => (
                            <div key={spec} className="flex items-center">
                                <input
                                    id={`oriented_specialty_${spec}_modal_edit`}
                                ...
                                />
                                <label htmlFor={`oriented_specialty_${spec}_modal_edit`} className="ml-2 text-sm text-gray-700">
                                    {spec}
                                </label>
                            </div>
                        ))}
            ...
            <FormField 
                ...
                name="notes" 
                error={errors.notes} 
                isRequiredFromUser={!isGP && !isSpecialistUser} 
            ...
            {isGP && (
                 <FormField label="" name="hors_champ_specialite_caravane">
                    <div className="flex items-center">
                        <input type="checkbox" name="hors_champ_specialite_caravane" id="hors_champ_specialite_caravane_modal_edit" checked={formData.hors_champ_specialite_caravane || false} onChange={handleChange} className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" />
                        <label htmlFor="hors_champ_specialite_caravane_modal_edit" className="ml-2 text-sm text-gray-700">Hors champ de spécialité de la caravane</label>
                    </div>
                </FormField>
            )}

            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                <FormField label="Diagnostic (Optionnel)" name="diagnostic" error={errors.diagnostic} >
                ...
                <FormField label="Orientation (Notes Générales)" name="orientation" error={errors.orientation} isHidden={isSpecialistUser}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 pt-2" hidden={isSpecialistUser}>
                <FormField label="Date du rendez-vous" name="date_rdv" error={errors.date_rdv} isHidden={isSpecialistUser}>
                    <input
                        type="date"
                        name="date_rdv"
                        id="date_rdv"
                        ...
                        onChange={handleChange}
                        className={`w-full px-3 py-2 border ${errors.date_rdv ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500`}
                    />
                </FormField>
                <FormField label="Date du prochain rendez-vous" name="date_prochain_rdv" error={errors.date_prochain_rdv} isHidden={isSpecialistUser}>
                    <input
                        type="date"
                        ...
                        onChange={handleChange}
                        className={`w-full px-3 py-2 border ${errors.date_prochain_rdv ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500`}
                    />
                </FormField>
            </div>
            ...
            {(isSpecialistUser) ? (
              <div className="pt-4 mt-4 border-t">
                <h3 className="text-md font-medium text-gray-700 mb-2">Actions et Suivi</h3>
                 <div className="space-y-3">
                    <FormField label="" name="scanner_effectue" error={errors.scanner_effectue}>
                        <div className="flex items-center">
                            <input type="checkbox" name="scanner_effectue" id="scanner_effectue_spec_edit" checked={formData.scanner_effectue} onChange={handleChange} className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" />
                            <label htmlFor="scanner_effectue_spec_edit" className="ml-2 text-sm text-gray-700">Bénéficier d’un scanner – dans Hôpital</label>
                        </div>
                    </FormField>
                    <FormField label="" name="orientation_chu" error={errors.orientation_chu}>
                        <div className="flex items-center">
                            <input type="checkbox" name="orientation_chu" id="orientation_chu_spec_edit" checked={formData.orientation_chu} onChange={handleChange} className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" />
                            <label htmlFor="orientation_chu_spec_edit" className="ml-2 text-sm text-gray-700">Suivi CHU</label>
                        </div>
                    </FormField>
                    <FormField label="" name="suivi_necessaire" error={errors.suivi_necessaire}>
                        <div className="flex items-center">
                            <input type="checkbox" name="suivi_necessaire" id="suivi_necessaire_spec_edit" checked={formData.suivi_necessaire} onChange={handleChange} className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" />
                            <label htmlFor="suivi_necessaire_spec_edit" className="ml-2 text-sm text-gray-700">Rendez-vous prochain</label>
                        </div>
                    </FormField>
                    {formData.suivi_necessaire && (
                         ...
                         >
                            <input
                                type="date"
                                name="date_prochain_rdv"
                                id="date_prochain_rdv_spec_edit"
                                value={formData.date_prochain_rdv || ''}
                                onChange={handleChange}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 items-center pt-2">
                    <FormField label="" name="scanner_effectue" error={errors.scanner_effectue} isHidden={false}>
                        <div className="flex items-center">
                            <input type="checkbox" name="scanner_effectue" id="scanner_effectue" checked={formData.scanner_effectue} onChange={handleChange} className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" />
                            <label htmlFor="scanner_effectue" className="ml-2 text-sm text-gray-700">Scanner Effectué</label>
                        </div>
                    </FormField>
                    <FormField label="" name="orientation_chu" error={errors.orientation_chu} isHidden={false}>
                        <div className="flex items-center">
                            <input type="checkbox" name="orientation_chu" id="orientation_chu" checked={formData.orientation_chu} onChange={handleChange} className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" />
                            <label htmlFor="orientation_chu" className="ml-2 text-sm text-gray-700">Orientation CHU</label>
                        </div>
                    </FormField>
                    <FormField label="" name="suivi_necessaire" error={errors.suivi_necessaire} isHidden={false}>
                        <div className="flex items-center">
                            <input type="checkbox" name="suivi_necessaire" id="suivi_necessaire" checked={formData.suivi_necessaire} onChange={handleChange} className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" />
                            <label htmlFor="suivi_necessaire" className="ml-2 text-sm text-gray-700">Suivi Nécessaire</label>
                        </div>
                    </FormField>
                </div>
            )}
            ...
            <FormField label="Statut de la Consultation" name="statut" error={errors.statut} isRequiredFromUser={!isGP && !isSpecialistUser} isHidden={isGP || isSpecialistUser}>
          ...
          <div className="flex justify-end pt-4 space-x-3">
            ...
            <button type="submit" disabled={isLoading || isDataLoading}
              ...
              ) : ( <><i className="fas fa-save mr-2"></i>Mettre à Jour</>)}
            </button>
          </div>
        </form>
      </div>
